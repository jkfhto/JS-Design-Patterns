...
- **职责链:**<br>
定义：使多个对象都有机会处理请求，从而避免请求的发送者和接收者之间的耦合关系，将这些对象连成一条链，并沿着这条链传递该请求，直到有一个对象处理它为止<br>
类型：```行为型```<br>
适用场景：1：一个请求的处理需要多个对象当中的一个或多个协作处理；<br>
优点：1：请求的发送者和接收者之间解耦；&nbsp;&nbsp;&nbsp;&nbsp;2：动态组合职责链；&nbsp;&nbsp;&nbsp;&nbsp;<br>
缺点：1：职责链太长或处理时间过长，会影响性能；&nbsp;&nbsp;&nbsp;&nbsp;2：职责链有可能过多<br>
- **命令模式:**<br>
定义：将请求封装成对象，以便使用不同请求，对请求排队或者记录请求日志，来实现撤销，重做等操作<br>
类型：```行为型```<br>
适用场景：1：有时候需要向某些对象发送请求，但是并不知道请求的接收者是谁，也不知道被请求的操作是什么。此时希望用一种松耦合的方式来设计程序，使得请求发送者和请求接收者能够消除彼此之间的耦合关系；<br>
优点：1：请求的发送者和接收者之间解耦；&nbsp;&nbsp;&nbsp;&nbsp;2：容易扩展新命令；&nbsp;&nbsp;&nbsp;&nbsp;<br>
缺点：1：命令的无限扩展会增加类的数量，提高系统实现复杂度；<br>
